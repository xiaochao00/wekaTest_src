 ...
 *
 */
public class MIAttributeEval extends ASEvaluation implements AttributeEvaluator, OptionHandler {
	private double[] m_MIs;
	/** Treat missing values as a seperate value */
	private boolean m_missing_merge;

	public MIAttributeEval() {
		resetOptions();
	}
...
//		    }
		...
		// Compute info gains
		m_MIs = new double[data.numAttributes()];
		for (int i = 0; i < data.numAttributes(); i++) {
			if (i != classIndex) {
				m_MIs[i] = WekaComputeUtil.computeMaxMI(counts[i], classCounts);
				// System.out.println(m_MIs[i]);
			}
		}
	}
